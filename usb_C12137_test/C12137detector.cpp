...
int C12137_detector::openDevice(){
	...
	if (devh == NULL) error_msg("USB open error\n C12137 device not found");
...
int C12137_detector::setInterface(int interfaceNum){
	...
	if (error!=0) error_msg("Claiming Interface");
...
void C12137_detector::readSensorData(unsigned short data[]){
	int length = HDSIZE+DATASIZE;
	unsigned char buffer[length*2];
	//unsigned char read_values[length*2];
	int received;
	//clearing the buffer
	memset(buffer,0,length*2);
	unsigned int timeout = 0; // 0 = unlimited time ->blocking call
	//before the transfer i clear the halt/stall condition of the breakpoint
	//error = libusb_clear_halt 	(devh, usbBulkEpIn);

	//getDeviceConfig();


	error = libusb_bulk_transfer (devh, usbBulkEpIn, buffer,length*2, &received,timeout);
	//should do some error checking.....later...
	switch (error){
	case 0:
		break;
	...
	case LIBUSB_ERROR_PIPE:
		printf("if the endpoint halted");break;// return;
	case LIBUSB_ERROR_OVERFLOW:
	    printf("if the device offered more data, see Packets and overflows "); return;
	case LIBUSB_ERROR_NO_DEVICE:
		printf("if the device has been disconnected"); return;
	default:
		printf("ha pasado algo ruaro..."); return;
	}
	...
	printf("datos leidos %d, error code %d",received, error);
