...
QRETURN CQuickUsb::Open() {
    if (QuickUsbOpen(&m_hDevice, m_devName) == 0) {
        ...
        QuickUsbGetLastDriverError(&m_lastDriverError);
        return false;
    }
...
QULONG CQuickUsb::GetLastDriverError() {
    QULONG lastError;
    
    QuickUsbGetLastDriverError(&lastError);
    return lastError;
}
...
QULONG CQuickUsb::LastDriverError() {
...
////////////////////////////////////
QRETURN CQuickUsb::AllocateDataBuffer(PQBYTE *buffer, QULONG bytes) {
    if (QuickUsbAllocateDataBuffer(buffer, bytes) == 0) {
        return false;
    }
...
QRETURN CQuickUsb::FreeDataBuffer(PQBYTE *buffer) {
    if (QuickUsbFreeDataBuffer(buffer) == 0) {
        return false;
    }
...
QRETURN CQuickUsb::StopStream(QBYTE streamID, QBOOL immediate) {
    // Don't politely open as we must already be open for this call

    if (QuickUsbStopStream(m_hDevice, streamID, immediate) == 0) {
        QuickUsbGetLastError(&m_lastError);
        QuickUsbGetLastDriverError(&m_lastDriverError);
        if (!immediate) {
            // Only close politely when were certain the stream has stopped
            ClosePolitely();
        }
        return false;
    }
